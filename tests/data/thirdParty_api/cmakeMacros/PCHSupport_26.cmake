...
MACRO(_PCH_WRITE_PCHDEP_CXX _targetName _include_file _dephelp)
  ...
  if(EXISTS ${${_dephelp}})
	MESSAGE( STATUS "Copy ${${_dephelp}}")
	FILE(WRITE  ${${_dephelp}}_New
...
")
	...
	# todo
	SET( FROM "${${_dephelp}}_New" )
	SET( TO "${${_dephelp}}" )
	EXECUTE_PROCESS(
		COMMAND ${CMAKE_COMMAND}
		-E copy_if_different ${FROM} ${TO}
		)
  else(EXISTS ${${_dephelp}})
	MESSAGE( STATUS "Write ${_dephelp}")
	  FILE(WRITE  ${${_dephelp}}
"#include \"${_include_file}\" 
int testfunction()
{
...
MACRO(GET_NATIVE_PRECOMPILED_HEADER _targetName _input)
	...
	if(CMAKE_GENERATOR MATCHES Visual*)
		...
		if(EXISTS ${${_targetName}_pch})
			FILE(WRITE ${${_targetName}_pch}_New ${_dummy_str})
			# Check if contents is the same, if not rewrite
			# todo
			SET( FROM "${${_targetName}_pch}_New" )
			SET( TO "${${_targetName}_pch}" )
			EXECUTE_PROCESS(
				COMMAND ${CMAKE_COMMAND}
				-E copy_if_different ${FROM} ${TO}
				)
